...
class TestModbusEventReader(unittest.TestCase):
    """
    Test suite for the ModbusEventReader class, focusing on event confirmation.
    """
    ...
    def test_request_events_with_confirmation(self, mock_wait_for_response, mock_send_command):
        """
        Test requesting event notifications with confirmation from the Modbus device.
        ...
        mock_wait_for_response.side_effect = [True, True, True, True, True, True]
        self.event_reader.serial_port.read.side_effect = [
            ...
            b'\xFF' * 9 + b'\xC8\x46\x11\x00\x01\x04\x00\x0F\x00\x00\xEE\x60',
        ...
        self.assertEqual(len(events), 1)
        self.assertEqual(events[0]['device_id'], 200)
        self.assertEqual(events[0]['event_count'], 1)
        self.assertEqual(events[0]['flag'], 0)
        self.assertEqual(events[0]['event_data_len'], 1024)
        self.assertEqual(events[0]['event_type'], 3840)
        self.assertEqual(events[0]['event_payload'], 238)

        events = self.event_reader.request_events(1, 100, 200, 0)
